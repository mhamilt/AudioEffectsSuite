...
//==============================================================================
class SimpleDeviceManagerOutputLevelMeter : public Component,
                                            public Timer
{
public:
	SimpleDeviceManagerOutputLevelMeter (AudioDeviceManager& m)
        : manager (m), level (0)
    {
        startTimer (50);
        manager.enableOutputLevelMeasurement (true);
    }

    ~SimpleDeviceManagerOutputLevelMeter()
    {
        manager.enableOutputLevelMeasurement (false);
    }
    ...
    void timerCallback() override
    {
        if (isShowing())
        {
            const float newLevel = (float) manager.getCurrentOutputLevel();
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SimpleDeviceManagerOutputLevelMeter)
};
        ...
        if (AudioDeviceSelectorComponent* parent = findParentComponentOfClass<AudioDeviceSelectorComponent>())
        {
            ...
            if (outputDeviceDropDown != nullptr)
            {
                ...
                if (testButton != nullptr)
                {
					...
					//======MY ADDITION======
					outputLevelMeter->setBounds(row.removeFromRight(testButton->getWidth()));
					//======MY ADDITION======
...
private:
    ...
    ScopedPointer<Component> inputLevelMeter,outputLevelMeter;
        ...
        if (setup.maxNumInputChannels > 0 && type.hasSeparateInputsAndOutputs())
        {
            if (inputDeviceDropDown == nullptr)
            {
				...
				addAndMakeVisible (outputLevelMeter
                    = new SimpleDeviceManagerOutputLevelMeter (*setup.manager));
            }
